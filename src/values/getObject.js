...
export default function GetType(newData) // constuctor
{

    if (!(this instanceof GetType)) { // without new
    ...
    this.or = function (orData = {}) {
        ...
        switch (this.newData) {
            case null:
                returnedArr = orData;
                errorAlert("NULL", orData);
                break;
            case undefined:
                returnedArr = orData;
                errorAlert("undefined", orData);
                break;
        }

        if (_.isNumber(this.newArr)) {
            returnedArr = orData;
            errorAlert("Number", orData);
        }
        if (_.isString(this.newArr)) {
            returnedArr = orData;
            errorAlert("String", orData);
        }
        if (_.isArray(this.newArr)) {
            returnedArr = orData;
            errorAlert("Array", orData);
        }
        return returnedArr;
    }
}